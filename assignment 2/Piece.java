...
public class Piece {
	...
	public Piece computeNextRotation() {
		...
		for(TPoint point : body){
			newPoint[index] = new TPoint(height - 1 - point.y, point.x);
			index++;
		}
		Piece ans = new Piece(newPoint);
		return ans;
	}
	...
	public boolean equals(Object obj) {
		...
		if(Arrays.equals(other.body,this.body)) return true;
		return false;
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		Piece next;
		while(true){
			next = currPiece.computeNextRotation();
			if(next.equals(root)){
				currPiece.next = root;
				break;
			}
